...
> 日期：2026-06-28（Phase A 已实现；2026-06-29 追加 Phase B 当前模板持久化；2026-06-30 追加 Phase C 预加载）
> 分支：基于 `feature/template-classifier` 新建分支实现
> 状态：Phase A/B 已完成并 review；Phase C 预加载已实现

## 1. 背景与目标

> **需求映射**（原始三需求 → 本设计覆盖点）：
> 1. 企业"岗位标准化" AI 教练系统 → 现有 chat 系统本体（不在本设计范围）。
> 2. 7 类模板 + 1 通用问答模板，路由 agent（实际上线的教练）按上传文件选择对应 agent，无新文件上传时保持当前 agent → §3 母版注册表/`resolve_master` 路由 + §4.5 `current_template_id` 跨轮保持。
> 3. 每个 agent 有 system/user/assistant 多轮提示词，最好支持预加载 → §3.1 母版格式（chat history JSON）+ §3.2 `MASTER_MESSAGES_CACHE` 预加载。
...
| 预加载 | 进程启动时把 8 份母版内容读入内存缓存（`MASTER_MESSAGES_CACHE`），请求时从缓存克隆取出，省去每轮磁盘 I/O + JSON 解析（需求 3"预加载"） |
...
- **预加载缓存**（需求 3）：`MASTER_MESSAGES_CACHE: dict[str | None, list[ChatMessage]]`——进程启动时（`main.py` lifespan）遍历 `master/`，把 8 份母版解析为 `ChatMessage` 列表缓存（键 = document_id 或 None；路径→键约定 `{D1..D7}.history.json`→`D1..D7`、`_generic.history.json`→`None`，由 `_master_key_from_path` 解析）。`load_master_messages` 优先从缓存克隆返回（用现有 `_clone_context_messages`，避免多会话共享同一可变列表），缓存未命中时再读磁盘并回填缓存。新增 `preload_master_messages(entries, logger)`（启动调用，幂等清空重建）+ `template_prompt_service.iter_master_entries()`（提供 `(key, path)` 对，与 `MASTER_REGISTRY` 同源，保证键集一致）。
...
7. chat.py: master_messages = load_master_messages(resolution.master_path, LOGGER)
   → 命中预加载缓存（`MASTER_MESSAGES_CACHE`）则直接克隆返回；未命中读盘并回填缓存（见 §3.2 预加载）
...
### 6.5 新增预加载测试（`tests/unit/test_context_service_master.py`）

- `preload_master_messages` 把所有存在母版缓存入 `MASTER_MESSAGES_CACHE`
- 缺失/空解析文件跳过（空解析 warning）
- 二次调用幂等（清空后重建，旧键不留）
- `load_master_messages` 命中缓存返回**克隆**（调用方修改返回值不污染缓存）
- 缓存未命中读盘并回填
- 通用母版路径映射到 `None` 键
...
**Phase C（追加，Task 12–13）—— 预加载（需求 3）**：
12. `context_service`：`MASTER_MESSAGES_CACHE` + `preload_master_messages(entries, logger)` + `load_master_messages` 改为缓存优先（克隆）·未命中读盘回填；`template_prompt_service.iter_master_entries()` 提供 `(key, path)` 对
13. `main.py` lifespan：`validate_master_registry(LOGGER)` 之后调 `preload_master_messages(iter_master_entries(), LOGGER)`；`tests/conftest.py` 隔离 fixture 清 `MASTER_MESSAGES_CACHE`；补预加载单元测试（见 §6.5）

## 9. 备选方案对比（决策记录）

> 问题：是否把 8 份母版各自做成"常驻 agent"，上层用 router agent 调用并保持状态？

**前提澄清**：LLM API 调用本身无状态（每次 `messages → text` 往返）。"agent 常驻内存"在 API 层能真正常驻的只有三样——① prompt/配置、② 对话历史、③ 工具集。本设计方案 A 已缓存 ①（`MASTER_MESSAGES_CACHE`）、持久化 ②（会话 + DB `current_template_id`），③ 不需要。故"常驻"的边际收益需从 ②历史如何组织 / ③是否 per-模板工具 中找，而非"换 agent 叫法"。

**方案定义**：
- **A（已采用）**：单 agent + 动态母版前缀。`resolve_master` 选母版 → `_build_model_messages` 前缀拼接 → 单次 `_call_llm`。
- **B1**：单 agent + 按 `template_id` 分桶对话历史（8 条独立辅导线程，切模板=切线程）。**不必多 agent 即可实现。**
- **B2（讨论中的"常驻 agent"）**：router agent + 8 个 specialist agent 对象常驻，router 用 LLM/tool-calling 委托。

**对比**：

| 维度 | A（已采用） | B2（多 agent 编排） |
|---|---|---|
| 每轮主 LLM 调用 | 1 次（带附件 +1 次分类器） | router 1 次 + specialist 1 次 = 2 次 |
| 路由决策 | `template_classifier`（已有，确定性文件分类） | router 再用 LLM 决策已被分类器解决之事 → 冗余 |
| "常驻"实质 | 母版数据缓存 + provider 侧 prompt cache | agent 对象常驻，但 API 无状态 → 常驻的仍是 prompt/历史，与 A 同 |
| 状态隔离 | 单会话单历史，切模板历史连续 | 每 agent 独立历史 → 切模板割裂对话；共享历史 → 退化为 A |
| 模板切换 UX | 换 system 前缀，"换教练继续聊"，连续 | 独立历史则旧上下文丢失，**违背 Phase B"无新附件须保持当前教练"硬需求** |
| 多会话并发 | 无状态，天然并发，缓存只读克隆 | agent 单例持可变状态 → 多会话竞争/串扰，需 per-session 实例或加锁 |
| 加第 N 个模板 | 加 1 份 master JSON + 注册表 1 行，零代码 | 定义新 agent 对象 + router 注册 + handoff 配置 |
| 调试可观测 | 单次调用链 | 多 agent 链路，需 trace/span |
| 现有代码契合 | 已是 `_call_llm(messages)` 无 tools，原生 | 需新增整个 agent 抽象层，改动大 |

**关键判断**：

1. **"agent 常驻"在 API 层是伪命题。** LLM 不持有会话句柄，常驻 agent 常驻的只是 system prompt 和（你选择存哪的）历史。这两样 A 都已缓存/持久化。B2 多出的 agent 对象每次调用仍要把 prompt+history 重发 API，与 A 单次调用等价——多出的只有 router 那一跳。除非 agent 带**专属工具**或**独立状态机**，否则常驻无收益。

2. **真正成本驱动是 D5 那份 738KB / 409 条消息的巨型母版，不是架构。** 每轮重发这份前缀的 token 成本，A 与 B2 都要付。正确杠杆是 **provider 侧 prompt caching**（OpenAI/Anthropic 均支持前缀缓存），A 直接可用、零架构改动；B2 同样依赖它，且因 router 多一跳反而更贵。

3. **B2 状态模型与硬需求冲突。** Phase B 要求"无新附件须保持当前教练持续回复"。B2 若每 specialist 自带独立历史，切模板=丢上下文，正破坏连续性；若 router 维护单一历史再转发，则 B2 严格退化为 A + 多余转发层。

**B 系列何时才划算**（任一触发再评估）：
- 每个教练需**不同工具集**（D5 查库、D1 调外部 API）——此时 agent 封装 (prompt+tools) 才有意义。
- 需要**多 agent 协作**（一教练起草、另一教练审核）。
- 需 router **自主判断何时切换**，而非用户上传文件被动触发。
- 需求变为"**8 条独立辅导线程并行**"——此为 B1，单 agent 按 `template_id` 分桶 history 即可实现，不必多 agent。

**结论**：维持 A。当前三条需求（路由选教练 / 无新文件保持当前教练 / 多轮 prompt 预加载）均不触发 B 系列任一条件。B2 在当前需求下为纯增复杂度、增延迟、增调用成本，且其独立历史模型与 Phase B 硬需求冲突。未来若出现上述触发条件，优先评估 **B1（分桶 history，单 agent）** 这一最小改动路径，而非一步到 B2 全套编排框架。
