...
6. chat.py: 若 resolution.document_id 与 session.current_template_id 不同 → 更新 session.current_template_id（内存 + DB）
...
- 第 3 步 `resolve_master(attachments=[], current_template_id=session.current_template_id)`：若 `current_template_id` 非空 → 返回 `ok` + 该模板母版路径（沿用）；否则 → `ok, _generic`。其余同主流程。**不再每轮无条件回退通用母版。**

### 4.5 当前模板状态（新增）

- `chat_sessions.current_template_id`（String，可空，取值 D1..D7）。
- 写入时机：仅识别成功（`resolution.status=="ok"` 且 `document_id` 为具体 D1..D7）时更新——即带附件且分类命中。无附件时不改写（保持现有模板）。拦截时不改写。
